"""
Evaluate In-Context Learning (Zero-shot and Few-shot) for Mistral-7B-Instruct.

This script evaluates the base Mistral model on the test set using:
...
def evaluate_icl(model, tokenizer, test_examples: List[Dict], train_examples: List[Dict], 
    ...
    for ex in tqdm(test_examples, desc=f"Evaluating {num_shots}-shot"):
        ...
        # Build prompt
        prompt = format_prompt_mistral(selected, inst, inp)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate ICL for Mistral")
    ...
    print(f"\n{'='*60}")
    print(f"Evaluating {shot_label} ICL for Mistral")
    print(f"{'='*60}\n")
    ...
    output_file = output_dir / f"icl_mistral_{shot_label.replace('-', '_')}.json"
