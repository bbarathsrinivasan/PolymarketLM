"""
Evaluate fine-tuned Mistral model on test set.

This script loads the fine-tuned Mistral model (with LoRA adapter) and evaluates it.
...
def main():
    ...
    parser.add_argument("--adapter_path", type=str, default="models/checkpoints/Polymarket-7B-LoRA")
    ...
    print(f"\n{'='*60}")
    print(f"Evaluating Fine-tuned Mistral Model")
    print(f"{'='*60}")
    ...
    # Print summary
    print(f"\n{'='*60}")
    print(f"RESULTS: FINE-TUNED MISTRAL")
    print(f"{'='*60}")
    ...
    output_file = output_dir / "finetuned_mistral.json"
